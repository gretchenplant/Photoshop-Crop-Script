...
//FoldMaker Product Image Creation Tool
//Version 1.0
//Unauthorized duplication or sharing of this tool is strictly prohibited.


alert("Welcome to the FoldMaker Product Image Creation Tool.\n\nThis tool will take all of the images in a designated folder\nand create a Hamburger, Hot Dog, and Portrait Fold version of each one of them automatically. The tool will walk you through the process step by step so just follow the instructions!\n\nThis tool runs only the following image file types:\n.png, .jpg, .jpeg.\n\nAdditionally, you need to run any of your remove-background or trimming processes **before** running this program. Images that aren't perfectly centered will produce unexpected results.");
    ...
    if(numOfFiles < 1){
        ...
        for (var i = 0; i < numOfFiles; i++){
            ...
            if(!(fileNameLength >= 198)){
                resizecanvasHDF(docRef);
                resizecanvasHF(docRef);
                resizecanvasPF(docRef);
...
function resizecanvasHDF(docRef){ //Create Hot Dog Fold file from source image
    docRef.resizeCanvas(docRef.width.value * .5, docRef.height.value, AnchorPosition.MIDDLERIGHT); //HotDog Fold the source image
...
function resizecanvasHF(docRef){ //Create Hamburger Fold file from source image
    docRef.resizeCanvas(docRef.width.value, docRef.height.value * .5, AnchorPosition.TOPCENTER); //Hamburger Fold the source image
...
function resizecanvasPF(docRef){ //Create Portrait fold from source image
    docRef.resizeCanvas(docRef.width.value * .66, docRef.height.value, AnchorPosition.MIDDLELEFT); //Cut the image by a third from the right side
    docRef.resizeCanvas(docRef.width.value * .5, docRef.height.value, AnchorPosition.MIDDLERIGHT); //Cut the image now in half from left side to get middle third of the image
    docRef.resizeCanvas(docRef.width.value, docRef.height.value * .4, AnchorPosition.TOPCENTER); //Cut the image now in half from the bottom up to create portrait fold
